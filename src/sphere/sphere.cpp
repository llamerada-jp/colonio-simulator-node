...
static std::uniform_real_distribution<double> rand_speed(0, 1.0 / 500.0);

Sphere::Sphere(colonio::Colonio&c, const Config& config, Logger& logger) : Base(c, config, logger, 1000) {
}

void Sphere::setup() {
  c.connect(config.get<std::string>("seed.url", "ws://localhost"), config.get<std::string>("seed.token", ""));
...
void Sphere::step() {
  ...
  c.set_position(current_x, current_y);
